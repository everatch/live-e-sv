function sv_showNowForm(lang,id,size_w,size_h,bc,bc_1,bc_2,bc_3,bc_4,bc_c,tc_1,tc_2,tc_3,tc_4,tc_c,code) {	
	size_h = 120;
	var html = "";
	
	var time = data[id][0][0];
	var temp = data[id][0][1];
	var humi = data[id][0][2];
	var pres = data[id][0][3];
	var rainfall = data[id][0][4];
	var winddir = data[id][0][5];
	var windspe = data[id][0][6];
			
	var winddirImg = windDir_toText(lang,windspe,winddir,"image");
								
	var now = new Date();
	var month = now.getMonth();
	var comment = getComment(lang,temp,humi,pres,rainfall,windspe,(month+1));
	comment = comment.split("<br/>").join("&nbsp;");
	
	var dush = "'";
	
	if(lang == "jp"){
		html += '<div style=" height:74px; font-family:Arial, Helvetica, sans-serif; text-align:center; font-size:11.5px; overflow:hidden;">'+
			'<div style="width:52%; height:44%; font-size:15px; color:'+tc_1+'; padding:3px 0; background-color:'+bc_1+'; float:left;">気温 <sv_api2_a style="font-size:30px; font-weight:500;">'+temp+'</sv_api2_a> ℃</div>'+
			'<div style="width:48%; height:60%; padding:3px 0; color:'+tc_2+'; background-color:'+bc_2+'; float:right;">湿度 <sv_api2_a style="font-size:18px">'+humi+'</sv_api2_a>%<br/>雨量 <sv_api2_a style="font-size:18px">'+rainfall+'</sv_api2_a> mm/h</div>'+
			'<div id="sv_api2_winddir" style="width:50%; height:35%; padding:7px 0; color:'+tc_3+'; background-color:'+bc_3+'; float:left;">風速 <sv_api2_a style="font-size:18px">'+windspe+'</sv_api2_a>m/s '+'<img src="'+winddirImg+'"></img>'+'</div>'+
			'<div style="width:50%; height:25%; padding:3px 0; color:'+tc_4+'; background-color:'+bc_4+'; float:right;">気圧 <sv_api2_a style="font-size:17px">'+pres+'</sv_api2_a> hPa</div></div>'+
			'<div style="height:21px; text-align:left; font-size:13px; color:'+tc_c+'; overflow:hidden; background-color:'+bc_c+'; position:relative; top:3px;"><nobr id="sv_api2_nowdata" style="position:relative; left:'+size_w+'px; top:4px;">'+comment+'</nobr></div>'+
			'<div style="position:relative; top:3px; font-size:10px; font-family:Arial, Helvetica, sans-serif;"><sv_api2_a style="float:left;">Sensor Viewer</sv_api2_a><sv_api2_a style="float:right;">| Data on '+time+'</sv_api2_a></div>'; 
	}else if(lang == "en"){
		html += '<div style=" height:74px; font-family:Arial, Helvetica, sans-serif; text-align:center; font-size:11.5px;" overflow:hidden;>'+
			'<div style="width:52%; height:44%; font-size:12px; color:'+tc_1+'; padding:3px 0; background-color:'+bc_1+'; float:left;">Temp <sv_api2_a style="font-size:30px; font-weight:500;">'+temp+'</sv_api2_a> ℃</div>'+
			'<div style="width:48%; height:60%; padding:3px 0; color:'+tc_2+'; background-color:'+bc_2+'; float:right;">Humi <sv_api2_a style="font-size:18px">'+humi+'</sv_api2_a>%<br/>Rain <sv_api2_a style="font-size:18px">'+rainfall+'</sv_api2_a> mm/h</div>'+
			'<div id="sv_api2_winddir" style="width:50%; height:29%; padding:7px 0; color:'+tc_3+'; background-color:'+bc_3+'; float:left;">Wind <sv_api2_a style="font-size:18px">'+windspe+'</sv_api2_a>m/s '+'<img src="'+winddirImg+'"></img>'+'</div>'+
			'<div style="width:50%; height:25%; padding:3px 0; color:'+tc_4+'; background-color:'+bc_4+'; float:right;">Pres <sv_api2_a style="font-size:17px">'+pres+'</sv_api2_a> hPa</div></div>'+
			'<div style="height:21px; text-align:left; font-size:13px; color:'+tc_c+'; overflow:hidden; background-color:'+bc_c+'; position:relative; top:3px;"><nobr id="sv_api2_nowdata" style="position:relative; left:'+size_w+'px; top:4px;">'+comment+'</nobr></div>'+
			'<div style="position:relative; top:3px; font-size:10px; font-family:Arial, Helvetica, sans-serif;"><sv_api2_a style="float:left;">Sensor Viewer</sv_api2_a><sv_api2_a style="float:right;">| Data on '+time+'</sv_api2_a></div>'; 
	}
	if(!code){
		document.write('<div style="height:'+(size_h-9)+'px; width:'+(size_w-4)+'px; background-color: '+bc+'; padding:2px 2px;"><div style="position:absolute; height:'+(size_h-9)+'px; width:'+(size_w-4)+'px;">'+html+'<script>sv_api2_comment_move("'+size_w+'","'+size_w+'");</script></div><div style="position:absolute; z-index:999; cursor:pointer;" onClick="window.open('+dush+'http://www.live-e-sv.info/'+dush+');"><img src="http://live-e-sv.info/developers/api/img/filter.png" height="'+(size_h-9)+'" width="'+(size_w-4)+'"></div></div>');
	}else if(code == "recode"){
		return '<div style="height:'+(size_h-9)+'px; width:'+(size_w-4)+'px; background-color: '+bc+'; padding:2px 2px;"><div style="position:absolute; height:'+(size_h-9)+'px; width:'+(size_w-4)+'px;">'+html+'<script>sv_api2_comment_move("'+size_w+'","'+size_w+'");</script></div><div style="position:absolute; z-index:999; cursor:pointer;" onClick="window.open('+dush+'http://www.live-e-sv.info/'+dush+');"><img src="http://live-e-sv.info/developers/api/img/filter.png" height="'+(size_h-9)+'" width="'+(size_w-4)+'"></div></div>';
	}
}

function sv_api2_comment_move(left_move_num,size_w){
	
	if(left_move_num < - document.getElementById("sv_api2_nowdata").innerText.length * 13){
		left_move_num = size_w;
	}
	
	left_move = left_move_num + "px";
	document.getElementById("sv_api2_nowdata").style.left = left_move;
	
	left_move_num -= 2.5;
	
	setTimeout("sv_api2_comment_move('"+left_move_num+"','"+size_w+"')", 50);
}

function getHTML(lang,id,num,bc_1,bc_2,cellSize,month) {	
    ...
    for (var i = 0 ;i<num ;i++) {
		...
		var comment = getComment(lang,temp,humi,pres,rainfall,windspe,month);
		
		if(lang == "jp"){
			html += '<div style="background-color: '+backcolor+'; padding:5px 10px;);"><table style="text-align:left; font-size:13px;">'+
					'<tr><td width="'+cellSize+'px">気温：'+temp+' ℃</td>'+
					'<td>気圧：'+pres+' hPa</td></tr>'+
					'<tr><td width="'+cellSize+'px">湿度：'+humi+' %</td>'+
					'<td>雨量：'+rainfall+' mm/h</td></tr>'+
					'<tr><td width="'+cellSize+'px">風速：'+windspe+'m/s</td>'+
					'<td>風向：'+winddirText+'</td></tr></table>'+
					'<div style="text-align:left; font-size:13px;">'+comment+'</div>'+
					'<div style="text-align:right; font-size:13px;"><br/>'+time+'</div></div>'; 
		}else if(lang == "en"){
...
function sv_showForm(text,lang,id,num,size_w,size_h,bc_t,tc_t,bc_b,tc_b,bc_1,bc_2,tc,code){
	...
	var now = new Date();
	var month = now.getMonth();
	var html = getHTML(lang,id,num,bc_1,bc_2,cellSize,(month+1));
...
function Rannum(num){
	var randnum = Math.round( Math.random() * (num-1) );
	return randnum;
}

function getComment(lang,temp,humi,pres,rainfall,windspe,month){
	var comment = "";
	...
	if(lang == "jp"){
		...
		if(temp < 1){
			if(rainfall == 0 || rainfall == "---"){
				text_array = Array(
					'<br/>とても寒いですね、路面凍結にはお気をつけください！',
					'<br/>こんなに寒いときは家でぬくぬくしてみかんを…',
					'<br/>いや～寒い、寒いな...やっぱ寒いよー！',
					'<br/>ストーブはイイネ～！',
					'<br/>いつまで続くんだろ...この寒さ　{{ (>_<) }}',
					'<br/>一日中布団かぶっていたい　((´д｀))',
					'<br/>さむ{{{{(極+ω+寒)}}}}ガタッ',
				...
				text_array = Array(
					'<br/>雪が降ってますね、交通情報をしっかり確認しましょう。',
					'<br/>雪(*´ー｀)ﾉ ｡ﾟ※ﾟ｡ﾟﾟ｡ﾟ※｡┐(´ー｀*)ﾉだぁ');
			}else{
				text_array = Array(
					'<br/>大雪です！交通情報はしっかり確認してください！');
			}
		}else if(temp < 5){
			text_array = Array(
				'<br/>かなり寒いですね、風邪にはお気をつけてください。',
				'<br/>とても寒いです、路面凍結にはお気を付けください！');
		}else if(temp < 15){
			if(humi > 30){
				text_array = Array(
					'<br/>寒いです、手洗いうがいは大切に！',
					'<br/>こたつぅ…ZZzz..o(-ε-*√￣￣＼',
					'<br/>朝も冷え込む時期になりましたね、体調管理はしっかり行ってください！',
					'<br/>寒いですね、風邪をひかないようにしっかり服を着込みましょう！',
					'<br/>こういう寒いときはコタツがほしいですね…');
			}else{
				text_array = Array(
					'<br/>インフルエンザには特にお気をつけください！手洗いうがいは大切です！',
					'<br/>肌寒いですね、寒くないような格好で出かけましょう。',
					'<br/>寒いですね、風邪をひかないようにしっかり服を着込みましょう！',
					'<br/>こういう寒いときはコタツがほしいですね…',
					'<br/>クリーム、乾燥したお肌にやさしく～');
			}
			...
			if(rainfall == 0 || rainfall == "---"){
				text_array = Array(
					'<br/>涼しいでしょう、外で遊べる絶好のチャンスです！',
					'<br/>少し寒いですね、風邪にはご注意を！',
					'<br/>風に当たると寒いね～',
					'<br/>寒いと感じることもありますね、薄すぎない服装で出かけてください！');
			}else{
				text_array = Array(
					'<br/>涼しいでしょう、雨が降っているので室内で遊びましょう！',
					'<br/>雨が降っています、傘はお忘れなく！');
			}
		}else if(temp < 30){
			if(rainfall == 0 || rainfall == "---"){
				text_array = Array(
					'<br/>少し熱いですね、十分熱中症の危険性がありますのでお気をつけください！',
					'<br/>少し熱いですね、こんなときでも熱中症の危険性がありますのでお気をつけください！',
					'<br/>紫外線も気になりますね、曇りでも気になる方は十分気をつけてください！');
			}else{
				text_array = Array(
					'<br/>雨が降っています！風邪にはご注意を…',
					'<br/>一時的な大雨にはご注意を！');
			}
		}else if(temp < 35){
			if(humi < 30){
				text_array = Array(
					'<br/>暑いです！涼しい服装でお出かけしましょう！',
					'<br/>カラカラとした天気です！熱中症には気をつけて！',
					'<br/>とても暑いですね！水分補給は忘れずに！');
			}else{
				text_array = Array(
					'<br/>蒸し暑いです！湿度も高いので、除湿をしましょう！',
					'<br/>むしむしとした天気です、いつも以上に暑く感じるでしょう。');
			}
		}else if(temp > 35){
				text_array = Array(
					'<br/>猛暑ですよ！暑さ対策をしっかり！熱中症を防ぐために、水分補給をこまめにしましょう！',
					'<br/>猛暑です！定期的に水分を十分に取って、過激な運動はお控えください！',
					'<br/>ほんとうに暑いです！熱中症には気をつけて！',
					'<br/>猛暑です！激しい運動はお控えください！');
		}

							var randnum = Rannum(text_array.length);
							comment = text_array[randnum];
		...
		if(randnum < 30){
			if(month == 12 || month <= 2){
				...
				season_array = Array(
							'<br/>今年も紅葉見に行くぞー！',
							'<br/>涼しくなって来たね＾＾',
							'<br/>食の秋、文化の秋、スポーツの秋...あとは？',
							'<br/>いい季節ですねー暑すぎないし、寒すぎない。');
			}
			randnum = Rannum(season_array.length);
			comment += season_array[randnum];
		}

		if(pres < 999 && windspe > 18){
			comment = "<br/>台風が来ているかもしれません、気象情報をこまめにチェックしてください。";
		}else if(pres < 999 && windspe > 24){
			comment = "<br/>台風が非常に強いです、お出かけ控えましょう！";
		}else if(pres < 999 && windspe > 40){
	...
	function windDir_toText(lang,windspe,winddir,code){
		var winddirText = "";
		var winddirImg = "";	
		
		if(lang == "jp" && code != "image"){
			if(windspe == 0){
				winddirText = "無風";
		...
		if(code == "image"){
			winddirImg = "http://live-e-sv.info/developers/api/img/winddirImg_" + winddirText.toLowerCase() + ".png";
			return winddirImg;
		}
		return winddirText;
	}
